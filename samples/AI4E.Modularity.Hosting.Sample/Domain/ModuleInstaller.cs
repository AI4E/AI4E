...
namespace AI4E.Modularity.Hosting.Sample.Domain
{
    public sealed class ModuleInstaller : IModuleInstaller
    {
        ...
        private async Task<IEnumerable<FileSystemModuleSource>> GetSourcesAsync(ModuleReleaseIdentifier moduleRelease, CancellationToken cancellation)
        {
            ...
            var sources = await Task.WhenAll(release.Sources.Select(p => p.ResolveAsync().AsTask()));
