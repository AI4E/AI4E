...
namespace AI4E.Routing.FrontEnd
{
    ...
    public sealed class LogicalServerEndPoint : ILogicalServerEndPoint, IAsyncDisposable
    {
        ...
        private async Task ReceiveInternalAsync(IMessage message, string address, CancellationToken cancellation)
        {
            ...
            {
                await ReceiveInitAsnyc(message, seqNum, corr, address, cancellation);
                return;
            }
        ...
        private async Task ReceiveInitAsnyc(IMessage message, int seqNum, int corr, string address, CancellationToken cancellation)
        {
            ...
            EncodeServerMessage(message, GetNextSeqNum(), corr: seqNum, MessageType.Response);
