...
{
    public sealed class MessagingEngine : IMessagingEngine
    {
        #region Fields
        ...
        public MessagingEngine(
            ...
            ILogger<MessagingEngine>? logger = null)
        {
        ...
        internal Task<IMessageRouter> GetMessageRouterAsync(CancellationToken cancellation)
        {
        ...
        {
            private readonly MessagingEngine _messageDispatcher;

            public RouteMessageHandler(MessagingEngine messageDispatcher)
            {
...
#pragma warning restore CA1031
        ...
        internal async ValueTask<IDispatchResult> DispatchAsync(
            DispatchDataDictionary dispatchData,
            bool publish,
            RouteEndPointScope remoteScope,
            RouteEndPointScope localScope = default,
            ...
            CancellationToken cancellation)
        {
            ...
            _logger?.LogInformation(
                $"End-point '{localEndPoint}': Dispatching message of type {dispatchData.MessageType} locally.");
            ...
            {
                _logger?.LogDebug(
                    $"End-point '{localEndPoint}': Dispatched message of type {dispatchData.MessageType} locally.");
            }
        }
...
#pragma warning restore CA1031
            ...
            CancellationToken cancellation)
        {
            ...
            var createSubscope = true;

            if (localScope.ClusterNodeIdentifier != ClusterNodeIdentifier.NoClusterNodeIdentifier
                && localScope.ScopeIdentifier != null)
            {
                MessageDispatcher? scopedMessageDispatcher;

                lock (_scopedDispatchersMutex)
                {
                    if (!_scopedDispatchers.TryGetValue(localScope, out scopedMessageDispatcher))
                    {
                        scopedMessageDispatcher = null;
                    }
                }

                // No such scope
                if (scopedMessageDispatcher is null)
                {
                    return new FailureDispatchResult("No such scope"); // TODO
                }

                serviceProvider = scopedMessageDispatcher.ServiceProvider;
                createSubscope = false; // TODO: This should be configurable.
            }

            using (ConditionallyScopeServiceProvider(serviceProvider, createSubscope, out var scopedServiceProvider))
            {
                var handler = handlerRegistration!.CreateMessageHandler(scopedServiceProvider);
        ...
        private static IDisposable? ConditionallyScopeServiceProvider(
            IServiceProvider serviceProvider,
            bool createScope,
            out IServiceProvider resultServiceProvider)
        {
            if (!createScope)
            {
                resultServiceProvider = serviceProvider;
                return null;
            }

            var serviceScope = serviceProvider.CreateScope();
            resultServiceProvider = serviceScope.ServiceProvider;
            return serviceScope;
        }
        ...
        // Use an immutable dictionary, in order to prevent locking when we need read-only access?
        private readonly Dictionary<RouteEndPointScope, MessageDispatcher> _scopedDispatchers
            = new Dictionary<RouteEndPointScope, MessageDispatcher>();
        private readonly object _scopedDispatchersMutex = new object();
        ...
        public IMessageDispatcher CreateDispatcher(IServiceProvider serviceProvider)
        {
            ...
            return new MessageDispatcher(this, serviceProvider);
        }

        internal bool RegisterScopedDispatcher(
            RouteEndPointScope scope, MessageDispatcher scopedMessageDispatcher)
        {
            Debug.Assert(scope != default);
            Debug.Assert(scopedMessageDispatcher != null);
            Debug.Assert(scopedMessageDispatcher.Engine == this);

            lock (_scopedDispatchersMutex)
            {
                return _scopedDispatchers.TryAdd(scope, scopedMessageDispatcher);
            }
        }

        internal void UnregisterScopedDispatcher(
            RouteEndPointScope scope, MessageDispatcher scopedMessageDispatcher)
        {
            Debug.Assert(scope != default);
            Debug.Assert(scopedMessageDispatcher != null);
            ...
            lock (_scopedDispatchersMutex)
            {
                _scopedDispatchers.Remove(scope, scopedMessageDispatcher);
            }
        }
